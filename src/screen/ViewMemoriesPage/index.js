...
function ViewSpeechPage() {
  ...
  const handleButtonClick = (index) => {
    ...
    // 제목과 북마크 상태를 결합하여 배열 생성
    const combined = titles.map((title, i) => ({
        title,
        content: contents[i],
        date: dates[i],
        isBookmarked: bookmark.includes(i) // 현재 북마크 여부
    }));
    ...
    // 북마크 상태 업데이트 (북마크된 항목만 유지)
    const newBookmarks = combined
        .map((item, i) => (item.isBookmarked ? i : -1)) // 북마크된 항목의 인덱스만 유지
        .filter(index => index !== -1); // -1은 필터링
    setBookmark(newBookmarks); // 새로운 북마크 상태 설정
};
